...
impl AnalogOscillator {
    ...
    // Enhanced PolyBLEP sawtooth with aggressive analog character
    fn shaped_sawtooth(&mut self, t: f32) -> f32 {
        ...
        // AGGRESSIVE ANALOG MODELING based on shape parameter
        if self.shape > 0.01 {
            // Waveform folding creates harsh harmonics (like analog distortion)
            let fold_amount = self.shape * 3.0;
            saw = (saw * fold_amount).tanh() / fold_amount.sqrt();
            
            // Step quantization for digital-aggressive sound
            let steps = 3.0 + self.shape * 15.0; // 3 to 18 steps
            saw = (saw * steps).round() / steps;
            
            // Asymmetric distortion adds bite
            if saw > 0.0 {
                saw = saw.powf(0.6 + self.shape * 0.7); // Make positive parts more aggressive
            } else {
                saw = -(-saw).powf(1.3 - self.shape * 0.5); // Keep negative parts sharper
            }
        }
